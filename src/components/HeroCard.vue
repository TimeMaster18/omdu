<template>
	<v-card class="card">
		...
		<v-card-text>
			...
			<div>
				<v-icon v-if="hero.range === Range.Melee">mdi-sword</v-icon>
				<v-icon v-else>mdi-bow-arrow</v-icon>
			...
			<v-row no-gutters class="mt-4 text-center">
				...
				<v-col>
					<ability-icon v-if="hero.abilities.fifth" :ability="hero.abilities.fifth" />
				</v-col>
			</v-row>

			<!-- Upgrades -->
			<h3 class="mt-4 mb-2">Upgrades</h3>
			<div>
				<upgrade-tier label="Level 2" :upgrade-tier="hero.upgrades.tier1" />
				<upgrade-tier label="Level 4" :upgrade-tier="hero.upgrades.tier2" />
				<upgrade-tier label="Level 7" :upgrade-tier="hero.upgrades.tier3" />
				<upgrade-tier label="Level 10" :upgrade-tier="hero.upgrades.tier4" />
			</div>
		</v-card-text>
	</v-card>
</template>
